...
1. The function is only ever invoked in tail-call position.
2. There are no non-tail calls or references to its closure.
...
| 2    | {a+b}       | {a+b, c-d}       |                      |
| 3    | {a+b, c-d}  | {a+b, c-d}       |                      |
| 4    | {a+b, c-d}  | {c-d}            | // a+b killed by a=5 |
...
| 1    | {a+b, x\*y} | {a+b, x\*y} |                           |
| 2    | {a+b, x\*y} | {a+b, x\*y} |                           |
| 3    | {a+b, x\*y} | {x\*y}      | // a+b used here kills it |
| 4    | {a+b, x\*y} | {x\*y}      |                           |
| 5    | {x\*y}      | ∅           |                           |
| 6    | ∅           | ∅           |                           |
...
1. **Basic‐block granularity:** Grouping statements into basic blocks reduces the number of graph nodes, decreasing iterations.
2. **Bit vectors:** Represent sets (e.g., gen/kill) as bit vectors so that union/intersection/kill operations become fast bitwise operations.
...
* **Condition:** Interfering neighbors of `v1` ∪ neighbors of `v2` < $K$.
* **Briggs heuristic:** Only coalesce if the combined interference degree is < $K$. Safe (no extra spill risk) but conservative (may miss some safe coalescings).
...
2. Maintain an active set of ranges currently in registers.
3. When a new range begins, expire old ones, then if free register exists assign it; else spill the range with latest end.
   **Advantage:** $O(n)$ time, simple.
...
1. **Data hazards** (RAW/WAW/WAR) ⇒ waiting for operand availability.
2. **Structural hazards** ⇒ resource conflicts (e.g., multiple loads).
...
* **Basic IC:**
  ...
  1. Initially, no target cached.
  2. On first call, do full method lookup and store in cache.
  3. Subsequent calls: if receiver type matches cache, direct jump; else fallback to lookup and update cache.
